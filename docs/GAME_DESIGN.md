...
- **Voidstep** (key 4, 14s cd, ground-target mobility): a short instant teleport, the same physical shape as the Mage's Blink — shorter range and a longer cooldown, since the Warlock leans on standing still rather than repositioning. Range 16 → 22 (40/80/140g). No damage.
  **Mastery** (600g/1600g, pick one): *Echoing Step → Doubled Step* — a second, then third, banked charge usable back-to-back before the full cooldown returns. *Void Collapse → Void Implosion* — the void erupts where you ARRIVE (not where you left, unlike the Mage's Arcane Rebound): 50 → 90 dmg and a 3-5-unit pull on anything nearby, an aggressive re-engage instead of a defensive parting shot.
